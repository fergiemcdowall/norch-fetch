...
program
  ...
  .option('-d, --directory <directory>', 'specify the fetch directory',
          String, 'fetch/')
  .option('-r, --urlregexp <urlregexp>',
          'an expression that matches the URLs to follow',
          String, '^n')
...
//  console.log(url);
  ...
  var protolib = urlBits.protocol == 'https:' ? 'https' : 'http';

  require(protolib).get(url, function(res) {
